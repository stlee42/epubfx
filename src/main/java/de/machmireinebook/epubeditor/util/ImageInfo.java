package de.machmireinebook.epubeditor.util;//optionally number of images, comments and physical resolution from
...
public class ImageInfo
{
    ...
    private boolean checkPnm(int id) throws IOException
    {
        ...
        final Format[] PNM_FORMATS = {Format.FORMAT_PBM, Format.FORMAT_PGM, Format.FORMAT_PPM};
    ...
    {
        if (format != null && format != Format.FORMAT_UNKNOWN)
        {
    ...
    public String getMimeType()
    {
        if (format != null)
        {
            if (format == Format.FORMAT_JPEG && progressive)
            {
                return "image/pjpeg";
            }
